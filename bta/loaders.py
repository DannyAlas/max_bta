...
from bta._types import TDTData, AllowedEvtypes, AllowedFormats, TankEventType, TDTDataHeader, SampleInfo, StoreType, TDTData, StoreType, DataFormatEnum, EventMarker, TDTEpoc, TDTNote, TDTStream, TDTSnip, TDTScalar, EventHeader
import numpy as np
import tdt
import logging
            ...
            for store_code in store_codes:
                ...
                if store_code["type_str"] == "epocs":
                    ...
                    if not store_code["name"] in [x.header.name for x in data.epocs]:
                        ...
                        curr_epoc.dform = store_code["dform"]
                        ...
                        curr_epoc.data = curr_epoc.data.view(np.uint64)
                        # if note is not None:
                    ...
                    if not store_code["name"] in [x.header.name for x in data.streams]:
                        ...
                        curr_stream.dform = store_code["dform"]
                    ...
                    if not store_code["name"] in [x.header.name for x in data.scalars]:
                        ...
                        curr_scalar.dform = store_code["dform"]
                        # if data.header.start_time is not None:
                        #     note = get_store_notes(
                        #         user_notes = heads[9, valid_ind].view(np.uint32),
            ...
            if not nodata:
                ...
                for f in range(len(arr)):
                    ...
                    tev_data = np.frombuffer(
                        tev.read(read_size * sz), AllowedFormats[snip.dform.name]
                    )
